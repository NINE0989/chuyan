...
OPENAI_ENV_VARS = {
    ...
    "OPENAI_BASE_URL": "API基础地址（可选，默认https://api.openai.com/v1）"
}
...
def set_env_vars(env_vars, persist=False):
    ...
    for key, value in env_vars.items():
        if value:  # 只设置非空值
            ...
            # 隐藏API密钥中间部分，仅显示前4位和后4位
            if key == "OPENAI_API_KEY" and len(value) > 8:
                masked = value[:4] + "****" + value[-4:]
            else:
                masked = value
            print(f"已设置 {key} = {masked}")
...
def main():
    print("=== OpenAI环境变量快速设置工具 ===")
    
    # 检查是否有配置文件
    config_path = Path.home() / ".openai.ini"
    ...
    # 交互式输入缺失的环境变量
    for key, desc in OPENAI_ENV_VARS.items():
        # 已有配置则跳过，否则提示输入（基础地址可选）
        if key not in env_vars or not env_vars[key]:
            prompt = f"请输入{desc}（可选，直接回车跳过）：" if key == "OPENAI_BASE_URL" else f"请输入{desc}："
            env_vars[key] = input(prompt).strip()
            # API密钥不能为空，强制检查
            while key == "OPENAI_API_KEY" and not env_vars[key]:
